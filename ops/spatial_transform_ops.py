...
def multilevel_crop_and_resize(features,
    ...
    with tf.name_scope('multilevel_crop_and_resize'):
        ...
        #tf.print("box width ", box_width, summarize=-1)
        #tf.print("Box Height ", box_height, summarize=-1)
        areas_sqrt = tf.sqrt(box_height * box_width)
        #tf.print("Areas Sqr ", areas_sqrt, summarize=-1)
        levels = tf.math.floordiv(tf.math.log(tf.divide(areas_sqrt, 224.0)), tf.math.log(2.0)) + 4.0
        #tf.print("Levels after tf.math ", levels, summarize=-1)
        if training:
        ...
        #tf.print("Levels after tf.min ", levels, summarize=-1)
        ...
        #tf.print("Expand ", tf.expand_dims(scale_to_level, axis=2), summarize=-1)
