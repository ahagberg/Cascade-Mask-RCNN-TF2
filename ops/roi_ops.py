    ...
    with tf.name_scope('multilevel_propose_rois'):
        ...
        for level in levels:
            ...
            with tf.name_scope('level_%d' % level) as scope:
                ...
                if use_tpu:
                  _propose_rois_fn = _propose_rois_tpu
                else:
                  _propose_rois_fn = _propose_rois_gpu
                this_level_scores, this_level_boxes = _propose_rois_fn(
                    this_level_scores,
                    this_level_boxes,
                    this_level_anchors,
                    height,
...
def box_outputs_to_rois(box_outputs, rois, class_outputs, image_info, weights):
  ...
  Args:
    ...
    class_outputs: `tensor` of classes that the box should be predicted for.
  ...
  class_outputs = tf.cast(class_outputs, tf.int32) #
  correct_class_one_hot = tf.tile(
    tf.expand_dims(
        tf.one_hot(class_outputs, num_classes, dtype=dtype), -1),
    [1, 1, 1, 4])
  ...
  new_rois = box_utils.decode_boxes(
      new_rois, rois, weights)

  height = tf.expand_dims(image_info[:, 0:1], axis=-1)
  width = tf.expand_dims(image_info[:, 1:2], axis=-1)
  new_rois = box_utils.clip_boxes(new_rois, height, width)
  return new_rois