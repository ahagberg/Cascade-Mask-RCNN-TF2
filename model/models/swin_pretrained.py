...
from tensorflow.keras.layers import Dense, Dropout, Conv2D, LayerNormalization, GlobalAveragePooling1D, Reshape
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            attn_mask = tf.where(attn_mask != 0, -100.0, tf.cast(attn_mask, dtype='float32'))
...
class SwinTransformerModel(tf.keras.Model):
    ...
    def __init__(self, model_name='swin_base_patch4_window12_384', include_top=False,
    ...
    def forward_features(self, x):
        ...
        for i,layer in enumerate(self.basic_layers):
            x , out_stage = layer(x)
            width = self.img_size // (4  * 2**i)
            stage_outputs.append(Reshape((width, width, self.embed_dim*(2**i)))(out_stage))
    ...
    def call(self, x):
        ...
        if self.include_top:
            x = self.head(x)
        return {i+2:stage for i, stage in enumerate(x)}


def swin_transformer(model_name='swin_base_384', num_classes=1000, include_top=False, pretrained=True, use_tpu=False, cfgs=CFGS):
    cfg = cfgs[model_name]
