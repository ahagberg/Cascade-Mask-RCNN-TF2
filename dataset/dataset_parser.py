...
def dataset_parser(data, mode, params, use_instance_mask, seed=None, regenerate_source_id=False):
    ...
    with tf.name_scope('parser'):
        ...
        if mode == 'eval':
            ...
            m = data['groundtruth_instance_masks']
            
            #print('instance_masks before: ', data['groundtruth_instance_masks'])
            ...
            # for i in range(features['cropped_gt_masks'].shape[0]):
            #     plt.imshow(features['cropped_gt_masks'][i,:,:])
            #     plt.show()
            ...
            #boxes = preprocess_ops.pad_to_fixed_size(boxes, -1, [MAX_NUM_INSTANCES, 4],  name='boxes')

            #classes = preprocess_ops.pad_to_fixed_size(classes, -1, [MAX_NUM_INSTANCES, 1],  name='classes')
            #print(" classes ", classes)
            #features['gt_boxes'] = boxes
            #features['gt_classes'] = classes
            #print('gt_classes before: ', classes)
            ...
            #print('gt_classes after ', features['gt_classes'])
...
def process_gt_masks_for_training(instance_masks, boxes, gt_mask_size, padded_image_size, max_num_instances):
    ...
    #print("cropped gt masks shape ", cropped_gt_masks.shape)
    # cropped_gt_masks = tf.reshape(cropped_gt_masks, [max_num_instances, -1])

    #print("gt mask size ", gt_mask_size)
